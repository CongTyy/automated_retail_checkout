...
class Annotator:
    ...
    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if self.pil or not is_ascii(label):
            ...
            for _box in box:
                p1, p2 = (int(_box[0]), int(_box[1])), (int(_box[2]), int(_box[3]))
...
def crop_vit(xyxy, im, gain=1.02, pad=10, square=False, BGR=False, save=True):
    xyxy = torch.tensor(xyxy).view(-1, 4)
    b = xyxy2xywh(xyxy)  # boxes
    if square:
        b[:, 2:] = b[:, 2:].max(1)[0].unsqueeze(1)  # attempt rectangle to square
    ...
    return crop, (int(xyxy[0, 0]), int(xyxy[0, 1]), int(xyxy[0, 2]), int(xyxy[0, 3]))
