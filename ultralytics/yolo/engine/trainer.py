...
class BaseTrainer:
    ...
    def _setup_train(self, rank, world_size):
        ...
        self.d1_optim = torch.optim.Adam(self.d1.parameters(), lr = self.d_lr, betas=(0.5, 0.999))

        self.d2 = D_layer6().to(self.device)
        self.d2.apply(init_weights)
        self.d2_optim = torch.optim.Adam(self.d2.parameters(), lr = self.d_lr, betas=(0.5, 0.999))
            ...
            for i, batch in pbar:
                ...
                with torch.cuda.amp.autocast(self.amp):
                    ...
                    if rgl:
                        ...
                        self.model.eval()
                        with torch.no_grad():
                            yfsource = self.model(batch['img'], gan = True)[0]
                            yftarget = self.model(batch_real['img'], gan = True)[0]
                        ...
                        c_loss = 0
                        self.critic_optim.zero_grad()
                        c_source = torch.mean(self.critic(yfsource.detach()))
                        c_target = torch.mean(self.critic(yftarget.detach()))
                        gp = self.calc_gradient_penalty(self.critic, yftarget.detach(), yfsource.detach(), self.device, LAMBDA = 10)
                        c_loss = -(-c_source + c_target) + gp
                        c_loss.backward()
                        self.critic_optim.step()
                        ...
                        self.writer.add_scalar('c_source', c_source.item(), self.step_gan)
                        self.writer.add_scalar('c_target', -c_target.item(), self.step_gan)
                        self.writer.add_scalar('gp', gp.item(), self.step_gan)
                        self.writer.add_scalar('c_loss', c_loss.item(), self.step_gan)
                        ...
                        preds, yfsource = self.model(batch['img'], gan = True, full = True)
                        # yolo loss
                        self.loss, self.loss_items = self.criterion(preds, batch)
                        # critic loss
                        c_domain = -torch.mean(self.critic(yfsource[0]))
                        scale = self.loss.item()/abs(c_domain.item())
                        self.loss = self.loss + c_domain                      
                        self.writer.add_scalar('loss', self.loss.item(), self.step_gan)
                        self.writer.add_scalar('scale', scale, self.step_gan)
    ...
    def build_optimizer(model, name='Adam', lr=0.001, momentum=0.9, decay=1e-5):
        ...
        return optimizer
    @staticmethod
    def calc_gradient_penalty(netD, real_data, fake_data, device, LAMBDA =10, domain = False):
        ...
        interpolates = autograd.Variable(interpolates, requires_grad=True) #if not domain else torch.concat([interpolates, fake_data, real_data], dim = 0).requires_grad_()
