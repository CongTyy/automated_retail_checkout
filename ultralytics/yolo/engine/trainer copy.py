...
# GAN
from ultralytics.nn.modules import Critic
...
class BaseTrainer:
    ...
    def _setup_train(self, rank, world_size):
        ...
        self.critic_model = Critic(3584).to(self.device)
        self.critic_optim = torch.optim.Adam(self.critic_model.parameters(), lr = self.c_lr, betas=(0.5, 0.999))
        self.combine_res = 40
    ...
    def _do_train(self, rank=-1, world_size=1):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                for _ in range(self.c_iters):
                    self.critic_model.zero_grad()
                    # for p in critic_model.parameters():
                    #     p.data.clamp_(-weight_cliping_limit, weight_cliping_limit)
                    preds, yf_fakes = self.model(batch["img"], gan = True) # yf: yolo features
                    ...
                    for f_r, f_f in zip(yf_reals, yf_fakes): # [16 64 160 160] [16 128 80 80] [16 256 40 40]  
                        kernel_size = f_r.size(3)//self.combine_res
                        yf_real.append(integrating(f_r.detach(), kernel_size))
                        yf_fake.append(integrating(f_f.detach(), kernel_size))

                    yf_real = torch.concat(yf_real, dim = 1) # 1792 x 40 x 40
                    yf_fake = torch.concat(yf_fake, dim = 1)

                    c_real = self.critic_model(yf_real, alpha = 1)
                    c_real = c_real.mean()
                    c_fake = self.critic_model(yf_fake, alpha = -1)
                    c_fake = c_fake.mean()
                    gradient_penalty = self.calc_gradient_penalty(self.critic_model, yf_real, yf_fake, device=self.device, LAMBDA=10)
                    c_loss = c_fake - c_real + gradient_penalty
                    c_loss.backward()
                    self.critic_optim.step()

                    self.writer.add_scalar("c_loss", c_loss, self.step_gan)
                    self.writer.add_scalar("c_fake", c_fake, self.step_gan)
                    self.writer.add_scalar("c_real", c_real, self.step_gan)
                    self.writer.add_scalar("gradient_penalty", gradient_penalty, self.step_gan)
                ...
                t1 = time.time()
                preds, yf_fakes = self.model(batch['img'], gan = True)
                yf_fake = []
                for f_f in yf_fakes: # [16 64 160 160] [16 128 80 80] [16 256 40 40]  
                    kernel_size = f_f.size(3)//self.combine_res
                    yf_fake.append(integrating(f_f, kernel_size))
                yf_fake = torch.concat(yf_fake, dim = 1)
                g_fake = self.critic_model(yf_fake.detach(), alpha = 1)
                g_fake = -g_fake.detach().mean()
                # t2 = time.time()
                # g_fake.backward(retain_graph=True)
                # print(f'forward: {t2 - t1} backward: {time.time() - t2} | ratio: {(time.time() - t2)/(t2-t1)}')
                ...
                self.writer.add_scalar("g_fake", g_fake, self.step_gan)
                self.step_gan += 1

                self.optimizer.zero_grad()
                (self.loss + g_fake).backward()
                self.optimizer.step()
    ...
    def build_optimizer(model, name='Adam', lr=0.001, momentum=0.9, decay=1e-5):
        ...
        return optimizer
    @staticmethod
    def calc_gradient_penalty(netD, real_data, fake_data, device, LAMBDA =10):
        ...
        alpha = alpha.expand(real_data.size(0), real_data.size(1), real_data.size(2), real_data.size(3)).to(device)
